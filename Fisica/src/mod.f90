...
real(dp) :: lambda,b, E_mean, E_error
real(dp) :: acc_rate
integer :: n_arr(9), M
...
do k = 1, 1
	...
	lambda = 1.0d0
	b = -1.0d0 !parámetro del término cuadrático del potencial 
	...
	do i=1, sweeps
		...
		if(i .ge. termalization .and. mod(i,steps) .eq. 0) then
			...
			! E_cont = E_cont + b*x(1)**2.0d0 + 3.0d0 * lambda * x(1)**4.0d0
			! E_cont_2 = E_cont_2 + (b*x(1)**2.0d0 + 3.0d0 * lambda * x(1)**4.0d0)**2.0d0

			print *, b*x(1)**2.0d0 + 3.0d0 * lambda * x(1)**4.0d0
...
function S_E(arreglo) result(resultado)
    ...
    do p = 1, (n-1)
        resultado = resultado + 0.5d0*((arreglo(p+1) - arreglo(p))/a)**2.0d0 + 0.5d0*b*arreglo(p)**2.0d0 + lambda*arreglo(p)**4.0d0
    end do

    ! Sumar el término adicional x_f^2
    resultado = resultado + 0.5d0*((arreglo(1) - arreglo(n))/a)**2.0d0 + 0.5d0*b*arreglo(n)**2.0d0 + lambda*arreglo(n) ** 4.0d0
...
function delta_S(xl, xi, xr, xp) result(sum)
    ...
    sum = sum + 0.5*b*(xp**2-xi**2) + lambda*(xp**4-xi**4)
    sum = a*sum
